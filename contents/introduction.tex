...
\begin{itemize}
  ...
  \item We report state-of-the-art quantitative and qualitative results on the task of human body parts localisation by employing a part-based AAM trained on the outline landmarks that are sampled by the dense AAM. Our proposed model outperforms all current state-of-the-art techniques that are trained on skeleton joints
  
  \item We show that the employed part-based AAM corrects the annotations that are currently provided for most major human body pose databases. Upon acceptance of the paper, we will make the corrected annotations publicly available.
