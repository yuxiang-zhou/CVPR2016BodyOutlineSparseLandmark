...
After establishing dense shape correspondences, we describe how dense Active Appearance Models (AAMs) can be built on top of that. We show that the proposed methodology can be applied for landmark localisation, even though not tailored for that task, achieving particularly good performance. We demonstrate the performance of the proposed methodology for describing objects that do not have consistent landmarks. Furthermore, We show that by sampling the shape of the dense AAM, a part-based AAM can be trained with its parts being in correspondence. We show that such a part-based AAM can used to produce state-of-the-art results in localisation of human body parts in challenging databases, outperforming state-of-the-art methods [..]. Finally, we show that the proposed part-based AAM can be used to provide consistent annotations for different body parts.
